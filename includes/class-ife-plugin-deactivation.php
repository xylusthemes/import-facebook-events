...
if ( ! class_exists( 'IFE_Plugin_Deactivation' ) ) {
	class IFE_Plugin_Deactivation {
		...
		function submit_plugin_deactivation_feedback() {
			if ( ! wp_verify_nonce( sanitize_text_field( $_REQUEST['nonce'] ), esc_attr( $this->prefix ) . 'plugin_deactivation_feedback' ) ) {
						...
						?>
							<div class="radio" style="padding:1px;margin-left:2%">
								<label for="<?php echo esc_attr( $key ); ?>">
									<input type="radio" name="<?php echo esc_attr( $this->prefix ); ?>deactivatation_reason_radio" id="<?php echo esc_attr( $key ); ?>" value="<?php echo esc_attr( $key ); ?>" required <?php if ( $key === 'confusing' ) { echo 'checked'; } ?> >
									<?php echo $deactivate_reason; ?>
								</label>
							</div>
						<?php } ?>
						<br>
						<textarea id="<?php echo esc_attr( $this->prefix ); ?>customer_query" name="<?php echo esc_attr( $this->prefix ); ?>customer_query" rows="4" placeholder="<?php _e( 'Write your query here', 'import-facebook-events' ); ?>"></textarea>
					</div>
				</form>
