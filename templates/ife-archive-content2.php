...
$eve_cats = array();
$event_categories = wp_get_post_terms( get_the_ID(), $ife_events->cpt->get_event_categroy_taxonomy() );
if ( ! empty( $event_categories ) ) {
	foreach ( $event_categories as $event_category ) {
		$eve_cats[] = '<a class="ife_event_cat" style="background-color:'. esc_attr( $accent_color ) .';" href="' . esc_url( get_term_link( $event_category->term_id ) ) . '">' . esc_attr( $event_category->name ) . '</a>';
	}
}
		...
		<div class="ife_event_style2">
			<div class="ife_event_thumbnail">
				<a href="<?php echo esc_url( $event_url ); ?>" <?php echo $target; ?> >
					<div class="ife_img_placeholder" style=" background: url('<?php echo esc_url( $image_url[0] ); ?>') no-repeat left top;"></div>
				</a>
